...
import { injectRuntimeHelpers } from "./core/src/common/helper-injector.ts";
import { getCachedPath } from "./core/src/common/hql-cache-tracker.ts";
import {
  basename,
  cwd as platformCwd,
  dirname,
...
// Create path object using platform-abstracted functions
const path = {
  basename,
  dirname,
  fromFileUrl: platformFromFileUrl,
  isAbsolute: platformIsAbsolute,
  join,
  normalize: platformNormalize,
  relative: platformRelative,
  resolve: platformResolve,
  toFileUrl: platformToFileUrl,
};
...
export async function transpile(
  ...
  // Inject runtime helpers using shared utility
  const injectionResult = await injectRuntimeHelpers(result.code, {
    generateSourceMap: options.generateSourceMap,
    sourceMap: result.sourceMap,
    forceIIFEWrap: true, // REPL mode always uses IIFE wrapping
  });

  // Return based on whether source maps were requested
  if (options.generateSourceMap && injectionResult.sourceMap) {
    return {
...
export async function run(
  ...
  const runtimeDir = platformResolve(platformCwd(), ".hql-cache/rt");
