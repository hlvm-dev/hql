...
(fn serialize-user (user)
  (const json (js-call JSON "stringify" user))
  json)
...
(fn validate-user (user)
  (const email (js-get user "email"))
  (const password (js-get user "password"))
...
(fn format-date (date)
  (const year (js-call date "getFullYear"))
  (const month (+ (js-call date "getMonth") 1))
  (const day (js-call date "getDate"))
  (+ year "-" 
     (if (< month 10) "0" "") month "-"
     (if (< day 10) "0" "") day))
...
(assert (=== (js-call cache "size") 2) "Map: size")
...
(var result (await (retry unreliable-action 5)))
(assert (=== result "success") "Retry: eventually succeeds")
