// src/hql/imports.ts - Enhanced error handling for imports
...
/**
 * Simple, fast hash function (cyrb53) for generating unique identifiers.
 * Used to prevent module ID collisions when paths differ only by special characters.
...
/**
 * Generate a unique, consistent internal module name from a path.
 * Uses hash-based IDs to prevent collisions between paths that differ only by special characters.
 *
 * Old approach (collision-prone):
 *   "./src/math.hql" and "./src-math.hql" both became "__module__src_math_hql"
 *
 * New approach (collision-free):
 *   "./src/math.hql"  -> "__module_math_1a2b3c4d"
 *   "./src-math.hql"  -> "__module_math_5e6f7g8h"
 */
...
/**
 * Check if a module path is an @hlvm/* stdlib package
 */
function isStdlibPackage(modulePath: string): boolean {
  return modulePath.startsWith("@hlvm/");
}
...
async function loadHqlModule(
  ...
  // Note: In parallel import scenarios (Promise.all), there's a theoretical race between
  // this check and the inProgressFiles.add() below. However, this is mitigated by:
  // 1. processedFiles.has() check above prevents duplicate final processing
  // 2. Imports are typically sequential in practice
  // 3. Even if duplicate work starts, the result is idempotent (same exports registered)
