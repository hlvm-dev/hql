...
import type { SymbolTable, SymbolInfo } from "../../transpiler/symbol_table.ts";
...
function buildBuiltins(): CompletionItem[] {
  ...
  return getAllKnownIdentifiers()
    .filter((id: string) => !snippetLabels.has(id))  // Don't duplicate snippets
    .filter((id: string) => !KERNEL_PRIMITIVES.has(id))  // Don't duplicate keywords
    .filter((id: string) => !id.startsWith("%"))  // Skip internal functions
    .filter((id: string) => !id.startsWith("_"))  // Skip private functions
    .map((id: string) => ({
      label: id,
