...
import { SymbolTable } from "../transpiler/symbol_table.ts";
import { HQLError } from "../../common/error.ts";
...
export function analyzeDocument(
  ...
  try {
    ...
    const hqlError = error instanceof HQLError ? error : null;
    if (hqlError?.sourceLocation) {
      const loc = hqlError.sourceLocation;
      errors.push({
        message: cleanErrorMessage(hqlError.message),
        code: hqlError.code?.toString(),
        range: {
          start: {
            line: loc.line ?? 1,
            column: loc.column ?? 1,
