...
function isQuotedSExpList(value) {
  return !!value &&
    typeof value === "object" &&
    value.type === "list" &&
...
export function first(coll) {
  ...
  if (isQuotedSExpList(coll)) {
    return coll.elements.length > 0 ? coll.elements[0] : undefined;
  }
...
export function rest(coll) {
  ...
  if (isQuotedSExpList(coll)) {
    return coll.elements.length <= 1
      ? SEQ_EMPTY
      : new ArraySeq(coll.elements, 1);
  }
...
export function seq(coll) {
  ...
  if (isQuotedSExpList(coll)) {
    return coll.elements.length === 0 ? null : new ArraySeq(coll.elements, 0);
  }
...
export function chunkedConcat(...colls) {
  ...
  function concatSeqs(seqs, idx) {
    if (idx >= seqs.length) return null;

    const first = seqToChunkedSeq(seqs[idx]);
    if (first == null) {
      return concatSeqs(seqs, idx + 1);
    }
    ...
    if (seqIsChunked(first)) {
      return chunkedLazySeq(() => {
        ...
        if (isSeqEnd(rest)) {
          const nextConcat = concatSeqs(seqs, idx + 1);
          if (nextConcat == null) {
            return seqChunkCons(chunk, null);
          }
          return seqChunkCons(chunk, nextConcat);
        }
        // Prepend rest of current seq, advance index for remaining seqs
        const tail = [rest];
        for (let i = idx + 1; i < seqs.length; i++) tail.push(seqs[i]);
        return seqChunkCons(chunk, chunkedLazySeq(() => concatSeqs(tail, 0)));
      });
    }
    ...
    return seqLazySeq(() => {
      ...
      const tail = [restFirst];
      for (let i = idx + 1; i < seqs.length; i++) tail.push(seqs[i]);
      return seqCons(fst, seqLazySeq(() => concatSeqs(tail, 0)));
    });
  }

  return chunkedLazySeq(() => concatSeqs(colls, 0));
}
...
// Return Promises (not lazy sequences). Designed for AI/IO composition.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/** Coerce nil/iterable → array. Returns [] for null (nil-punning). */
function __hql_toArray(coll) {
  if (coll == null) return [];
  return Array.isArray(coll) ? coll : Array.from(coll);
}

export async function asyncMap(f, coll) {
  const results = [];
  for (const item of __hql_toArray(coll)) results.push(await f(item));
  return results;
}

export async function concurrentMap(f, coll) {
  return Promise.all(__hql_toArray(coll).map(f));
}

export async function asyncFilter(f, coll) {
  const results = [];
  for (const item of __hql_toArray(coll)) if (await f(item)) results.push(item);
  return results;
}

export async function asyncReduce(f, init, coll) {
  let acc = init;
  for (const item of __hql_toArray(coll)) acc = await f(acc, item);
  return acc;
}

export async function asyncFlatMap(f, coll) {
  return (await asyncMap(f, coll)).flat();
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// AI PRIMITIVES — thin wrappers over globalThis.ai (set up by HLVM runtime)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

export function ai(prompt, options) {
  if (!globalThis.ai) throw new Error("ai requires HLVM runtime (globalThis.ai not available)");
  return globalThis.ai(prompt, options);
}

export function agent(prompt, options) {
  if (!globalThis.ai?.agent) throw new Error("agent requires HLVM runtime (globalThis.ai.agent not available)");
  return globalThis.ai.agent(prompt, options);
}
