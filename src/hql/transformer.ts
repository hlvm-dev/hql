...
// - TS2307 "Cannot find module '@hlvm/*'" - embedded packages aren't real modules
  ...
  try {
    ...
    if (result.typeErrors && result.typeErrors.length > 0 && options.showTypeWarnings !== false) {
      for (const err of result.typeErrors) {
        ...
        // Skip errors from internal @hlvm/* packages (not user code)
        const errorFile = err.file || "";
        if (errorFile.startsWith("@hlvm/") || errorFile.includes("/@hlvm/")) {
          continue;
        }
