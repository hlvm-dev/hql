  ...
  resetAfterCommit(rootNode) {
    ...
    if (COMMIT_LOG) {
      ...
      if (gap > 30 || reconcileMs > 20 || _createCount > 50) {
        ...
        appendFileSync(
          COMMIT_LOG,
          `${now.toFixed(1)} gap=${gap.toFixed(1)}ms reconcile=${
            reconcileMs.toFixed(1)
          }ms creates=${_createCount}\n`,
        );
      }
      _createCount = 0;
      if (now - _lastLog > 1000) {
        ...
        appendFileSync(
          COMMIT_LOG,
          `${now.toFixed(1)} commits=${_commits}/s maxGap=${
            _maxGapMs.toFixed(1)
          }ms\n`,
        );
        _commits = 0;
    ...
    if (COMMIT_LOG) {
      ...
      if (layoutMs > 20) {
        ...
        appendFileSync(
          COMMIT_LOG,
          `${_t0.toFixed(1)} SLOW_YOGA ${
            layoutMs.toFixed(1)
          }ms visited=${c.visited} measured=${c.measured} hits=${c.cacheHits} live=${c.live}\n`,
        );
      }
    }
