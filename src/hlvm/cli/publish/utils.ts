...
/**
 * HLVM project configuration interface
 */
export interface HlvmProjectConfig extends Record<string, unknown> {
  name: string;
  version: string;
  exports?: string;
  description?: string;
...
export function generatePackageMetadata(
  ...
  const packageJson = {
    ...
    description: config.description || `HLVM module: ${config.name}`,
    main: "mod.js",
    types: "mod.d.ts",
  ...
  const denoJson = {
    name: config.name.startsWith("@") ? config.name : `@${config.author || "hlvm"}/${config.name}`,
    version: config.version,
    exports: "./mod.ts",
    tasks: {
      test: "deno test",
 ...
 */
export async function ensureReadme(projectPath: string, config: HlvmProjectConfig): Promise<void> {
...
> **This is a template README automatically generated by [HLVM Publish](https://github.com/hlvm-dev/hlvm).**
...
This is a module published with [HLVM](https://github.com/hlvm-dev/hlvm).
...
## 🛠 Publishing with HLVM

To publish updates, run:

\`\`\`sh
hlvm publish <entry-file> [jsr|npm] [version] [--dry-run]
\`\`\`
See [HLVM Publish Guide](https://github.com/hlvm-dev/hlvm) for full details.
...
- [HLVM Documentation](https://github.com/hlvm-dev/hlvm)
