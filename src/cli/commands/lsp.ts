 ...
 * HQL LSP Command
 *
 * Starts the HQL Language Server for IDE integration.
 *
 * Usage:
 *   hql lsp           Start LSP server (stdio transport)
 *   hql lsp --stdio   Explicitly use stdio transport
 *   hql lsp --help    Show help
 */
...
DESCRIPTION:
  Starts the HQL Language Server Protocol (LSP) server for IDE integration.
...
EDITOR SETUP:
  ...
  VSCode / Cursor:
    Install the HQL extension from the marketplace, or configure manually:

    1. Install the HQL extension
  ...
  Emacs (with lsp-mode):
    (lsp-register-client
      (make-lsp-client
        :new-connection (lsp-stdio-connection '("hql" "lsp" "--stdio"))
  ...
  Helix:
    ...
    scope = "source.hql"
    file-types = ["hql"]
    language-server = { command = "hql", args = ["lsp", "--stdio"] }
...
EXAMPLES:
  hql lsp                    # Start LSP server with stdio
  hql lsp --stdio            # Explicitly use stdio transport
`);
}
