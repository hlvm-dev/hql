...
| **Skills Distribution/Lifecycle** | `createUserSkill()` + `importSkillPath()` + `installSkillFromGit()` + `removeSkill()` + `updateSkills()` + `checkSkills()` | `src/hlvm/agent/skills/install.ts`                                  | None                                                    |
