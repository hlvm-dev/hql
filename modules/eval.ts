  ...
  while (currentEnv) {
    for (const key in currentEnv.bindings) {
      ...
      if (
        ...
        enumCase.name in binding.value
      ) {
        if (result !== null)
...
function isLabel(arg: HQLValue): boolean {
  return arg.type === "symbol"
    ? arg.name.endsWith(":")
    : arg.type === "string"
      ? (arg.value as string).endsWith(":")
      : false;
}
...
function processLabeledArgs(fnVal: HQLFn, argVals: HQLValue[]): HQLValue[] {
  ...
  if (
    ...
    for (const k in argVals[0].value) {
      ...
      labelMap[key] = jsToHql((argVals[0].value as Record<string, any>)[k]);
    }
...
// Async core evaluation function
async function evaluateCoreAsync(ast: HQLValue, env: Env, realPath?: string): Promise<HQLValue> {
  if (ast.type === "list" && ast.value.length > 0) {
    const [head, ...rest] = ast.value;
    if (head.type === "symbol") {
      switch (head.name) {
        case "new": return await handleNewAsync(rest, env);
        case "quote": return rest[0] ?? makeNil();
        case "if": return await handleIfAsync(rest, env);
        case "def":
        case "defsync":
        case "defmacro":
          return await handleDefinitionForm(head.name as "def" | "defsync" | "defmacro", rest, env, evaluateAsync, head.name === "defsync", realPath);
        case "export": {
          if (rest.length !== 2)
        ...
        case "fn":
        case "fx": return makeFunctionLiteralWrapper(rest, env, head.name === "fx");
        case "defn":
        case "defx": return handleDefn(head.name, rest, env);
        case "defenum": return handleDefenum(rest, env);
        case "import": return await handleImportSpecialForm(rest, env, realPath);
      }
    }
...
// Sync core evaluation function
function evaluateCoreSync(ast: HQLValue, env: Env): HQLValue {
  if (ast.type === "list" && ast.value.length > 0) {
    const [head, ...rest] = ast.value;
    if (head.type === "symbol") {
      switch (head.name) {
        case "new": return handleNewSync(rest, env);
        case "quote": return rest[0] ?? makeNil();
        case "if": return handleIfSync(rest, env);
        case "def":
        case "defsync": return handleDefinitionForm(head.name as "def" | "defsync", rest, env, evaluateSync, head.name === "defsync");
        case "export": {
          if (rest.length !== 2)
            throw new Error("(export) expects exactly two arguments: string and value");
          const exportNameAst = rest[0];
...
function handleDefenum(rest: HQLValue[], env: Env): HQLValue {
  ...
  for (const c of rest.slice(1)) {
    ...
    enumObj[c.name] = { type: "opaque", value: Symbol(`${enumName}.${c.name}`) };
  }
...
/*────────────────────────────────────────────────────────────
  FUNCTION CALL HANDLERS (ASYNC & SYNC)
────────────────────────────────────────────────────────────*/
