# HLVM HQL VSCode Extension

Language support for HQL on HLVM, including syntax highlighting, autocompletion, and LSP features.
...
src/hql/transpiler/keyword/primitives.ts → KERNEL_PRIMITIVES, ALL_OPERATOR_NAMES
        ↓                                       ↓
src/hql/lsp/features/completion.ts     syntaxes/hql.tmLanguage.json
...
Embedded in HLVM binary (`hlvm lsp --stdio`). Source in `/src/hql/lsp/`:
...
**No completion?** Wait 1-2 sec after opening file. Check Output → "HLVM HQL Language Server".

**Server not starting?** Test: `hlvm lsp --stdio` or `deno run --allow-all ../src/hql/lsp/server.ts`
