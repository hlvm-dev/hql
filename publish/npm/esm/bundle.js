...
function makeFunctionLiteral(parts, env, isPure) {
    ...
    if (parts[0].type === "list" && parts[0].value.length > 0 && parts[0].value[0].type === "list")
        parts = parts[0].value.concat(parts.slice(1));
...
function handleDefn(formName, rest, env) {
    if (rest.length < 2)
        throw new Error("defn expects a name and a function definition");
    const nameSym = rest[0];
    if (nameSym.type !== "symbol")
        throw new Error("defn expects a symbol as function name");
...
function handleDefinitionForm(formName, rest, env, evalFn, markSync, realPath) {
    ...
    if (formName === "defmacro") {
        ...
        const params = rest[1].value.map((p) => {
            if (p.type !== "symbol")
                throw new Error("Macro parameter must be a symbol");
            return p.name;
        });
        const macroVal = {
            type: "function",
        ...
        if (head.type === "symbol") {
            switch (head.name) {
                ...
                case "export":
                    {
                        if (rest.length !== 2)
                            throw new Error("(export) expects exactly two arguments: string and value");
...
async function doImport(url, baseUrl) {
    ...
    if (modObj.default?.__hql_module)
        return modObj.default.__hql_module;
    if (modObj.__hql_module)
        return modObj.__hql_module;
...
function numericOp(op) {
    return (args) => {
        ...
        const nums = args.map((a) => {
            if (a.type !== "number")
                throw new Error(`Expected number in ${op}`);
            return a.value;
        });
        switch (op) {
            case "+":
...
const stdlibs = {
    ...
    keyword: hostFunc(([s]) => {
        if (!s || s.type !== "string")
            throw new Error("(keyword) expects one string");
...
async function buildImportMap(entryFile, cacheDir, visited = new Set()) {
    ...
    if (!absoluteFilePath.endsWith(".hql"))
        return mappings;
...
async function buildJsModule(inputDir) {
    ...
    if (hqlFiles.length === 0) {
        console.log("No .hql files found in " + outDir);
        return;
    }
    for (const hqlName of hqlFiles) {
        const hqlPath = join4(outDir, hqlName);
        const source = await readTextFile(hqlPath);
        const compiled = await compile(source, hqlPath, false);
        const outJS = hqlPath + ".js";
        await writeTextFile(outJS, compiled);
        console.log(`Transpiled ${hqlPath} -> ${outJS}`);
    }
    const indexTsPath = join4(outDir, "all_hql_modules.ts");
    const lines = hqlFiles.map((file) => {
        const baseName = file.replace(/\.hql$/, "");
        return `export * as ${baseName} from "./${file}.js";`;
    });
    await writeTextFile(indexTsPath, lines.join("\n"));
    console.log(`Created re-export file at ${indexTsPath}`);
    const bundlePath = join4(outDir, "hql_module.bundle.js");
    console.log(`Bundling modules into ${bundlePath}...`);
    ...
    if (!bundleStatus.success) {
        console.error("Failed to bundle HQL modules.");
  ...
  const entrypoint = Deno.env.get("DNT_ENTRYPOINT") || "./hql_module.bundle.js";
  console.log("Using DNT_ENTRYPOINT:", entrypoint);
  ...
  `.trim();
    const dntConfigPath = join4(outDir, "dnt.config.ts");
    await writeTextFile(dntConfigPath, dntConfigContent);
    console.log(`Created dynamic dnt.config.ts at ${dntConfigPath}`);
    setEnv("DNT_ENTRYPOINT", "./hql_module.bundle.js");
    console.log("Running dnt to produce Node-compatible package...");
    ...
    if (!dntStatus.success) {
        console.error("dnt build failed. Check dnt.config.ts or your environment.");
        exit(dntStatus.code);
    }
    console.log("dnt build succeeded. Node package is available in the ./npm folder.");
}
...
async function publishNpm(options) {
    ...
    if (!await exists(npmDistDir)) {
        console.error("No npm folder found. Did DNT fail?");
        exit(1);
    }
    const pkgJsonPath = join4(npmDistDir, "package.json");
    if (!await exists(pkgJsonPath)) {
        console.error("No package.json found in npm/. Did DNT fail?");
        exit(1);
    }
    ...
    console.log(`Updated package.json with name=${pkgName} version=${newVersion}`);
    const readmePath = join4(npmDistDir, "README.md");
    if (!await exists(readmePath)) {
        await writeTextFile(readmePath, `# ${pkgName}\n\nAuto-generated README for npm.`);
    }
    ...
    proc.close();
    if (!status.success) {
        console.error("npm publish failed. Make sure you're logged in or have permission.");
        exit(status.code);
    }
    console.log(`Package published successfully! https://www.npmjs.com/package/${pkgName}`);
}
async function publishJSR(options) {
    ...
    pkg.version = newVersion;
    const dirName = outDir.split("/").pop() || "hql-package";
    const defaultName = `@boraseoksoon/${dirName}`;
    let finalName = (options.name || defaultName).toLowerCase().replace(/_/g, "-");
    pkg.name = finalName;
    await writeTextFile(pkgJsonPath, JSON.stringify(pkg, null, 2));
    ...
    const jsrConfig = {
        name: finalName,
        version: newVersion,
        exports: "./esm/hql_module.bundle.js"
    };
    ...
    const status = await publishProcess.status();
