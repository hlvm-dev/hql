...
async function publishNpm(options) {
    ...
    for await (const entry of Deno.readDir(outDir)){
        if (entry.isFile && entry.name.endsWith(".hql")) {
            hqlFile = join4(outDir, entry.name);
            break;
        }
    }
    if (!hqlFile) {
...
const importMeta = {
    url: "file:///Users/seoksoonjang/dev/hql/modules/publish_jsr.ts",
    main: false
};
...
async function publishJSR(options) {
    ...
    for await (const entry of Deno.readDir(outDir)){
        if (entry.isFile && entry.name.endsWith(".hql")) {
            hqlFiles.push(join4(outDir, entry.name));
        }
    }
    ...
    const finalName = options.name || `@boraseoksoon/${basename3(outDir)}`;
    const jsrConfigPath = join4(outDir, "jsr.json");
    const jsrConfig = {
        name: finalName,
        version: pkgVersion,
        exports: "./mod.ts"
    ...
    const runtimeBundleTarget = join4(outDir, "hql_runtime.bundle.js");
