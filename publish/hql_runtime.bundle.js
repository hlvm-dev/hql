...
async function buildJsModule(options) {
    const sourceDir = resolve3(options.source);
    const outDir = resolve3(options.out);
    await mkdir(outDir, {
        recursive: true
    });
    const compiledFiles = [];
    ...
    for (const { outFile } of compiledFiles){
        const base = basename3(outFile);
        modTsContent += `export * from "./${base}";\n`;
    }
    const modTsPath = join4(outDir, "mod.ts");
  ...
  `;
    const modDtsPath = join4(outDir, "mod.d.ts");
    await writeTextFile(modDtsPath, modDtsContent);
    console.log("Generated mod.d.ts");
    const pkgName = options.name || `@boraseoksoon/${basename3(sourceDir)}`;
    ...
    const pkgPath = join4(outDir, "package.json");
...
async function publishNpm(options) {
    ...
    const pkgPath = join4(builtDir, "package.json");
...
const importMeta = {
    url: "file:///Users/seoksoonjang/dev/hql/modules/publish/publish_jsr.ts",
    main: false
};
async function generateModFiles(outDir) {
    const files = [];
...
async function publishJSR(options) {
    ...
    const finalName = options.name || `@boraseoksoon/${basename3(builtDir)}`;
    const jsrConfigPath = join4(builtDir, "jsr.json");
    const jsrConfig = {
        name: finalName,
        version: pkgVersion,
        exports: "./mod.ts"
    ...
    const readmePath = join4(builtDir, "README.md");
    if (!await exists(readmePath)) {
        await Deno.writeTextFile(readmePath, `# ${finalName}\n\nAutogenerated README for ${finalName}.\n`);
        console.log(`Generated README.md at ${readmePath}`);
    }
    const runtimeSource = new URL("../../hql.ts", importMeta.url).pathname;
    const runtimeBundleTarget = join4(builtDir, "hql_runtime.bundle.js");
    ...
    console.log(`Copying package contents from ${builtDir} to ${tempDir}...`);
    await copy(builtDir, tempDir, {
        overwrite: true
    });
...
async function startCmd(args) {
    ...
    if (Object.keys(importMap.imports).length > 0) {
        importMapPath = join4(projectRoot, "hql_import_map.json");
...
export { exportHqlModules as exportHqlModules, getHqlModule as getHqlModule };
