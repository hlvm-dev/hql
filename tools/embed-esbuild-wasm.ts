...
async function findEsbuildWasm(): Promise<Uint8Array> {
  ...
  for (const path of paths) {
    try {
      ...
      console.log(`✅ Found esbuild.wasm at: ${path}`);
      return data;
    } catch {
  ...
  console.log("📥 WASM not found in cache, downloading from npm...");
  ...
  console.log(`✅ Downloaded esbuild.wasm (${(data.length / 1024 / 1024).toFixed(2)} MB)`);
  return data;
}
...
export async function getEsbuildWasmModule(): Promise<WebAssembly.Module> {
  ...
  const binaryString = atob(ESBUILD_WASM_BASE64);
...
async function main() {
  console.log("🚀 Embedding esbuild WASM...");
  ...
  const base64 = uint8ArrayToBase64(wasmBytes);
  console.log(`📊 Base64 size: ${(base64.length / 1024 / 1024).toFixed(2)} MB`);

  // Generate output file
  console.log("✨ Generating esbuild-wasm-embedded.ts...");
