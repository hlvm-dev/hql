...
async function main() {
  ...
  console.log(`📊 Original WASM size: ${(wasmBytes.length / 1024 / 1024).toFixed(2)} MB`);

  // Compress
  console.log("🗜️  Compressing (gzip)...");
  const compressedBytes = await compress(wasmBytes);
  console.log(`📊 Compressed size: ${(compressedBytes.length / 1024 / 1024).toFixed(2)} MB`);
  console.log(`📉 Reduction: ${(100 - (compressedBytes.length / wasmBytes.length * 100)).toFixed(1)}%`);
