...
binaryTest("CLI ask: fixture-backed verbose transcript includes team coordination events", async () => {
  await withTempDir(async (dir) => {
    const port = await findFreePort();
    const result = await runCLI(
      "ask",
...
binaryTest("CLI ask: fixture-backed json transcript streams team coordination events", async () => {
  await withTempDir(async (dir) => {
    const port = await findFreePort();
    const result = await runCLI(
      "ask",
    ...
    assertEquals(
      lines.some((line) =>
        line.type === "agent_event" && line.event?.type === "team_task_updated"
      ),
      true,
    );
    assertEquals(
      lines.some((line) =>
        line.type === "agent_event" && line.event?.type === "team_plan_review_required"
      ),
      true,
    );
    assertEquals(lines.at(-1)?.type, "final");
...
binaryTest("CLI ask: natural language request triggers system-managed delegation and team coordination", async () => {
  await withTempDir(async (dir) => {
    const port = await findFreePort();
    const result = await runCLI(
      "ask",
