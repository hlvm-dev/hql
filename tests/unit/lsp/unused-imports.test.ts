...
(add 5 10)`;
  ...
  const result = detectUnusedImports(code, "test.hql");
...
Deno.test("UnusedImports - removes entire import when single symbol unused", () => {
  const code = `(import [foo] from "./utils.hql")
(print 1)`;
  const doc = createDoc(code);
...
(export [greet farewell])`;
  ...
  const result = detectUnusedImports(code, "middleware.hql");
