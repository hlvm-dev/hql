...
Deno.test({
  name: "worktree: creates worktree in .hlvm/worktrees/",
  async fn() {
    ...
    try {
      ...
      assertStringIncludes(info.worktreePath, ".hlvm/worktrees/");
      ...
      const readme = await platform.fs.readTextFile(`${info.worktreePath}/README.md`);
      assertStringIncludes(readme, "Test Repo");

      // Cleanup
      await removeAgentWorktree(info.worktreePath, info.worktreeBranch, info.gitRoot);
    } finally {
      await cleanDir(repo);
    }
  },
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      await removeAgentWorktree(info1.worktreePath, info1.worktreeBranch, info1.gitRoot);
    } finally {
      await cleanDir(repo);
    }
  },
