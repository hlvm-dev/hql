...
Deno.test("memory: memory_write and memory_search cover both memory and journal targets", async () => {
  await withTestEnv(async () => {
    ...
    assert(getValidFacts().some((fact) => fact.content.includes("tabs over spaces")));

    const prefSearch = await memorySearch({ query: "tabs spaces" }) as Record<string, unknown>;
    const journalSearch = await memorySearch({ query: "OAuth tokens" }) as Record<string, unknown>;
...
Deno.test("memory: sanitizer helper and tool path redact sensitive content before storage", async () => {
  await withTestEnv(async () => {
    ...
    await memoryWrite({
      content: "User SSN is 123-45-6789 and API key is sk_live_abcdefghijklmnopqrstuvwxyz",
      target: "memory",
    });
    ...
    assertEquals(stored.content.includes("sk_live_abcdefghijklmnopqrstuvwxyz"), false);
  });
});
...
Deno.test("memory: reuseSession refreshes memory without losing the system prompt", async () => {
  await withTestEnv(async () => {
    ...
    context.addMessage({ role: "system", content: "# Your Memory\nStale preference: vim keybindings" });

    const reused = await reuseSession(createAgentSession(context));
    const messages = reused.context.getMessages();

    assert(messages.some((message) => message.content === "You are an assistant."));
    assert(messages.some((message) => message.content.includes("Fresh preference: emacs keybindings")));
  });
});

Deno.test("memory: invalid tool inputs are rejected", async () => {
  await withTestEnv(async () => {
    await assertRejects(async () => await memoryWrite({ content: "", target: "memory" }));
    await assertRejects(async () => await memoryWrite({ content: "test", target: "invalid" }));
...
Deno.test("memory: memory_edit deletes sections and replaces text across facts", async () => {
  await withTestEnv(async () => {
    ...
    const deleted = await memoryEdit({ action: "delete_section", section: "Remove" }) as Record<string, unknown>;
    ...
    assertEquals(facts.some((fact) => fact.content.includes("Outdated info")), false);
...
Deno.test("memory: fact CRUD covers defaults, invalidation, search, touch, and category filters", async () => {
  await withTestEnv(async () => {
    ...
    insertFact({ content: "function foo(bar) returns baz", category: "reference" });
    ...
    assert(valid.some((fact) => fact.id === defaultId && fact.accessCount === 2));
    ...
    const row = getFactDb().prepare("SELECT * FROM facts WHERE id = ?").value<unknown[]>(customId);
    assert(row !== null);
  });
});
