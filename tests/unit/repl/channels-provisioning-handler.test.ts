...
Deno.test("channel provisioning handler rejects unknown channel", async () => {
  ...
  const complete = await handleChannelProvisioningComplete(
    new Request("http://test.local/api/channels/slack/provisioning/session/complete", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({}),
    }),
  ...
  const cancel = await handleChannelProvisioningCancel(
    new Request("http://test.local/api/channels/slack/provisioning/session/cancel", {
      method: "POST",
    }),
    { channel: "slack" },
  );
