...
Deno.test("pastes: filter by language", () => {
  ...
  const pastes = getPastes();
  const jsPastes = pastes.filter((p: { lang: string }) => p.lang === "javascript");
...
Deno.test("pastes: map to extract content", () => {
  ...
  const contents = getPastes().map((p: { content: string }) => p.content);
...
Deno.test("attachments: filter by type", () => {
  ...
  const images = getAttachments().filter((a: { type: string }) => a.type === "image");
...
Deno.test("conversation: filter by role", () => {
  ...
  const userTurns = getConversation().filter((t: { role: string }) => t.role === "user");
  assertEquals(userTurns.length, 3);

  const assistantTurns = getConversation().filter((t: { role: string }) => t.role === "assistant");
