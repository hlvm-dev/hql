...
for await (const entry of walk(coreLibPath, {
  ...
  if (entry.isFile) {
    ...
    const macroKey = `src/lib/${relativePath}`;
...
const macrosOutputPath = fromFileUrl(new URL("../src/lib/embedded-macros.ts", import.meta.url));
await Deno.writeTextFile(macrosOutputPath, macrosOutput);
console.log(`✅ Generated src/lib/embedded-macros.ts`);
