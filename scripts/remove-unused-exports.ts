/**
 * Remove unused exports identified by find-unused-exports.ts
 * This is a semi-automated removal tool
 */

// Map of files to exports that should be removed
...
async function removeExport(file: string, exportName: string): Promise<boolean> {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (regex.test(line)) {
        modified = true;
        // Skip this line and potentially related lines
        continue;
      }
...
async function main() {
  ...
  for (const [file, exports] of REMOVALS) {
    ...
    for (const exp of exports) {
      const success = await removeExport(file, exp);
      if (success) removed++;
      else failed++;
    }
  }
