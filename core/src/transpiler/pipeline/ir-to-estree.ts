...
/**
 * Get the runtime helper source code as a string
 */
function getHelperSource(name: string): string {
  const helpers: Record<string, string> = {
    '__hql_get': `function __hql_get(obj, key, defaultValue) {
    ...
    '__hql_getNumeric': 'const __hql_getNumeric = __hql_get;',
    '__hql_range': `function __hql_range(...args) {
    let start, end, step;
    if (args.length === 1) {
        [end] = args;
        start = 0;
    ...
    '__hql_toSequence': `function __hql_toSequence(value) {
    if (value === null || value === undefined) return [];
    if (Array.isArray(value)) return value;
    if (typeof value === "number") {
        const result = [];
        const step = value >= 0 ? 1 : -1;
        for (let i = 0; step > 0 ? i < value : i > value; i += step) result.push(i);
        return result;
    }
    ...
    '__hql_for_each': `function __hql_for_each(sequence, iteratee) {
    ...
    '__hql_hash_map': `function __hql_hash_map(obj) {
    return obj;
}`,
    '__hql_throw': `function __hql_throw(error) {
    throw error;
}`,
    '__hql_deepFreeze': `// __hql_deepFreeze is injected by runtime, not transpiler
const __hql_deepFreeze = typeof globalThis.__hql_deepFreeze !== 'undefined' ? globalThis.__hql_deepFreeze : (x => x);`
  };
...
/**
 * Parse helper source code to ESTree nodes
 */
function parseHelperToESTree(source: string): Statement[] {
  // Simple parser for our helper functions - converts string to ESTree
  // This is a minimal implementation that works for our specific helper format
  // In production, you'd use a full parser like acorn

  // For now, return empty - we'll implement this if needed
  // The key insight is that we should generate ESTree nodes programmatically
