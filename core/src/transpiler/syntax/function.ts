...
/**
 * Validate function call arguments and provide helpful guidance
 * Ensures users follow the new parameter style guidelines
 */
function validateFunctionCallArguments(
  funcName: string,
  args: HQLNode[],
): void {
  // First, check for removed named argument syntax
  detectRemovedNamedArgumentSyntax(args);

  // Check if user is trying to pass a hash-map as argument
  // This is allowed and will work, but we can add future validations here
  // For example, checking if it matches expected JSON map parameter syntax
}
