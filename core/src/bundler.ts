...
import { injectRuntimeHelpers } from "./common/helper-injector.ts";
...
async function processHqlEntryFile(
  ...
  // Get the cached path BEFORE injection so we can calculate correct import paths
  // Note: We get the path without writing yet (createDir: false)
  const tsOutputPath = await getCachedPath(
    resolvedInputPath,
    ".ts",
    { createDir: false }
  );

  // Inject runtime helpers for standalone bundles (NO IIFE wrapping for bundler)
  logger.log({
    text: "Injecting runtime helpers into transpiled code",
    namespace: "bundler",
  });
  const injectionResult = await injectRuntimeHelpers(jsCode, {
    generateSourceMap: !!sourceMap,
    sourceMap: sourceMap,
    forceIIFEWrap: false, // Bundler mode: simple prepending, no IIFE
    cachedFilePath: tsOutputPath, // Pass cached path for calculating relative imports
  });

  // Update code and source map after injection
  jsCode = injectionResult.code;
  if (injectionResult.sourceMap) {
    sourceMap = injectionResult.sourceMap;
  }

  // Now write the injected code to the cached path
...
async function postProcessBundleOutput(outputPath: string): Promise<void> {
  try {
    ...
    // Note: Runtime helpers are now injected during transpilation (see processHqlEntryFile)
    // No need for band-aid fixes here anymore
...
export async function transpileHqlInJs(
  ...
  try {
    ...
    // Inject runtime helpers (no IIFE for imported modules)
    const injectionResult = await injectRuntimeHelpers(processedContent, {
