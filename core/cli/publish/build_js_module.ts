import { prebundleHqlImportsInJs, transpileCLI } from "../../src/bundler.ts";
import {
  basename,
  dirname,
  ensureDir,
...
import { checkForHqlImports } from "../../src/common/utils.ts";
import {
  isHqlFile,
  isJsFile,
  isTypeScriptFile,
} from "../../src/common/import-utils.ts";
 ...
 */
async function addExternalDependenciesToPackageJson(
  distDir: string,
  externals: string[],
  verbose?: boolean,
): Promise<void> {
  ...
  // Version mapping for known external packages
  ...
  try {
    if (!await exists(packageJsonPath)) {
      if (verbose) {
        logger.debug(`package.json not found at ${packageJsonPath}, skipping dependency addition`);
      }
      return;
    }
    ...
    // Add each external package with its version
    for (const pkg of externals) {
      const version = packageVersions[pkg] || "latest";
      packageJson.dependencies[pkg] = version;

      if (verbose) {
        logger.debug(`Added dependency: ${pkg}@${version}`);
      }
    }
    ...
    console.log(`📦 Added ${externals.length} external dependencies to package.json`);
  } catch (error) {
    console.warn(
      `\n⚠️ Failed to add dependencies to package.json: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
