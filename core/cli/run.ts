...
/**
 * Create a temporary HQL file from an expression
 */
async function createTempFileFromExpression(
  expression: string,
  runDir: string,
): Promise<string> {
  const tempFilePath = `${runDir}/expr.hql`;
  const { writeTextFile } = await import("../src/platform/platform.ts");
  await writeTextFile(tempFilePath, expression);
  return tempFilePath;
}
