...
/**
 * Print welcome message with dragon ASCII art
 */
function printWelcome(): void {
  const dragon = `
${CYAN}            __${RESET}
${CYAN}           / _)${RESET}      ${RED}~${ORANGE}*${YELLOW}~${RESET}
${CYAN}    .-^^^-/ /${RESET}       ${RED}~${ORANGE}*${YELLOW}*${ORANGE}~${RESET}
${CYAN} __/       /${RESET}         ${ORANGE}~${YELLOW}*${RESET}
${CYAN}<__.|_|-|_|${RESET}
...
async function generateBindingCode(
  ...
  const declaration = isRedefinition ? "" : "var ";
  return {
    code: `${comment}${declaration}${name} = ${jsValue};\n`
  };
}
...
async function generateExpressionCode(
  ...
  return {
    code: `${comment}export const ${exportName} = ${transpiled};\n`,
    exportName
  };
}
